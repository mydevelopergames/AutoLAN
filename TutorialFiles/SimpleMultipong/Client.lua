...
-Currently this version is in Beta testing. Do not use this for production builds unless you have purchased AutoLAN. Please be advised that things may not be stable now.
		...
		while(broadcastMessage and type(broadcastMessage)=="string") do	
			...
			if(broadcastMessage) then
				if(broadcastMessage[1] and broadcastMessage[1]=="CoronaMultiplayer" and broadcastMessage[2]==applicationName) then --this is the protocol id				
					if(availibleServers[serverIP] == nil or availibleServers[serverIP].port ~= serverPort) then
